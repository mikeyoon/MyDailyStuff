...
export class SignupComponent extends React.Component<BaseProps> {
    ...
    renderSignupError() {
        if (this.props.store.authStore.registerError) {
            return <div className="alert alert-danger">{this.props.store.authStore.registerError}</div>;
        }
                ...
                <form onSubmit={this.onSubmit.bind(this)}>
                    ...
                    <div className={"form-group " + (this.errors["password"] ? 'has-error' : '')} key="2">
                        ...
                        <input className="form-control" id="password" name="password" type="password"
                               value={this.password} onChange={e => this.passwordChanged(e.target.value)}/>
                        {this.errors["password"] ? <span
                            className="help-block">{this.errors["password"]}</span> : null}
                    </div>
                    <div className={"form-group " + (this.errors["confirm"] ? 'has-error' : '')} key="3">
                        ...
                        <input className="form-control" id="confirm" name="confirm" type="password"
                                   value={this.confirm} onChange={e => this.confirmChanged(e.target.value)}/>
                        {this.errors["confirm"] ? <span
                            className="help-block">{this.errors["confirm"]}</span> : null}
                    </div>
